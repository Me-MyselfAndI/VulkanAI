...
def setup_gpt():
    API_URL = "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"
    ORG_URL = "org-v7zVnLJHsAUIJqpFrCXVIxzN"
    #     HEADERS = {
    #         "Authorization": f"Bearer {ORG_URL}",
...
def get_gpt_assessment(product, user_input):
    prompt = f"Given the user's preference of '{user_input}', does the product titled '{product}' match the criteria? Answer yes or no only"
    result = openai.ChatCompletion.create(model="gpt-4", messages=[{"role": "user", "content": prompt}])

    assessment = result.choices[0].message.content.strip().lower()
...
def filter_products(product_info, user_input):
    filtered_products = []
    for product in product_info:
        if get_gpt_assessment(product, user_input):
