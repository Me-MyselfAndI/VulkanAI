...
class Parser:
    def __init__(self, html=None, soup=None):
        ...
        self.dom = lxml.etree.HTML(str(self.soup))

    def find_marketplace_product_groups(self):
    ...
    def _find_deep_siblings(self, element, N, include_self=True):
        parent = element
        for i in range(N):
    ...
    def find_website_menu(self, likelihood_threshold=0.5, min_common_depth=0, max_common_depth=7, max_link_search_depth=5):
        ...
        def find_common_ancestral_path(element1, element2, min_depth=0, max_depth=10):
        ...
        for element in self.soup.find_all():
            ...
            if likelihood_score >= likelihood_threshold:
                ...
                if any(item['href'] == element.get('href') and item['onclick'] == element.get('onclick') for item in menu_items[nth_ancestor]['items']):
                    continue
        ...
        for i, el_i in enumerate(non_sibling_elements):
            for j, el_j in enumerate(non_sibling_elements[:i]):
                common_parent = find_common_ancestral_path(el_i['tag'], el_j['tag'], min_common_depth, max_common_depth)
                if common_parent:
